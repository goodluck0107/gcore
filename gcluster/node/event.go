...
// Event 获取事件类型
func (e *event) Event() gcluster.Event {
	return e.event
}
...
func (e *event) Parse(v interface{}) error {
	return gerrors.NewError(gerrors.ErrIllegalOperation)
}
...
func (e *event) GetIP() (string, error) {
	return e.node.proxy.GetIP(e.ctx, &gcluster.GetIPArgs{
		GID:    e.gid,
		Kind:   gsession.Conn,
		Target: e.cid,
	})
...
// Deliver 投递消息给节点处理
func (e *event) Deliver(args *gcluster.DeliverArgs) error {
	return e.node.proxy.Deliver(e.ctx, args)
}

// Reply 回复消息
func (e *event) Reply(message *gcluster.Message) error {
	return e.node.proxy.Push(e.ctx, &gcluster.PushArgs{
		GID:     e.gid,
		Kind:    gsession.Conn,
		Target:  e.cid,
		Message: message,
...
func (e *event) Disconnect(force ...bool) error {
	return e.node.proxy.Disconnect(e.ctx, &gcluster.DisconnectArgs{
		GID:    e.gid,
		Kind:   gsession.Conn,
		Target: e.cid,
...
func (e *event) NewMeshClient(target string) (gtransport.Client, error) {
