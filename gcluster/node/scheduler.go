...
func (s *Scheduler) spawn(creator Creator, opts ...ActorOption) (*Actor, error) {
	...
	act.events = make(map[gcluster.Event]EventHandler, 3)
	...
	if _, ok := s.load(act.Kind(), act.ID()); ok {
		s.mu.Unlock()
		return nil, gerrors.ErrActorExists
	}
...
func (s *Scheduler) bindActor(uid int64, kind, id string) error {
	...
	act, ok := s.load(kind, id)
	if !ok {
		return gerrors.ErrNotFoundActor
	}
...
func (s *Scheduler) dispatchRequest(ctx Context) error {
	...
	if uid == 0 {
		return gerrors.ErrMissDispatchStrategy
	}

	kind, ok := s.routes.Load(ctx.Route())
	if !ok {
		return gerrors.ErrUnregisterRoute
	}

	act, ok := s.loadActor(uid, kind.(string))
	if !ok {
		glog.Errorf("dispatch request failed, uid = %v route = %v kind = %v", uid, ctx.Route(), kind)
		return gerrors.ErrNotBindActor
	}
