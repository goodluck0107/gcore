...
// Event 获取事件类型
func (r *request) Event() gcluster.Event {
	return 0
}
...
func (r *request) BindGate(uid ...int64) error {
	...
	default:
		return gerrors.ErrIllegalOperation
	}
}
...
func (r *request) GetIP() (string, error) {
	...
	return r.node.proxy.GetIP(r.ctx, &gcluster.GetIPArgs{
		GID:    r.gid,
		Kind:   gsession.Conn,
		Target: r.cid,
	})
...
// Deliver 投递消息给节点处理
func (r *request) Deliver(args *gcluster.DeliverArgs) error {
	return r.node.proxy.Deliver(r.ctx, args)
}

// Reply 回复消息
func (r *request) Reply(message *gcluster.Message) error {
	switch {
	case r.gid != "": // 来源于网关
		return r.node.proxy.Push(r.ctx, &gcluster.PushArgs{
			GID:     r.gid,
			Kind:    gsession.Conn,
			Target:  r.cid,
			Message: message,
	...
	case r.nid != "": // 来源于其他Node
		...
		return r.node.proxy.Deliver(r.ctx, &gcluster.DeliverArgs{
			NID:     r.nid,
			UID:     r.uid,
			Message: message,
		})
...
func (r *request) Response(message interface{}) error {
	return r.Reply(&gcluster.Message{
		Route: r.message.Route,
		Seq:   r.message.Seq,
		Data:  message,
	})
...
func (r *request) Disconnect(force ...bool) error {
	...
	return r.node.proxy.Disconnect(r.ctx, &gcluster.DisconnectArgs{
		GID:    r.gid,
		Kind:   gsession.Conn,
		Target: r.cid,
...
func (r *request) NewMeshClient(target string) (gtransport.Client, error) {
