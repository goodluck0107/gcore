...
func (r *Router) AddRouteHandler(route int32, stateful bool, handler RouteHandler, middlewares ...MiddlewareHandler) {
	if r.node.getState() != gcluster.Shut {
		glog.Warnf("the node server is working, can't add route handler")
		return
	}
...
func (r *Router) SetDefaultRouteHandler(handler RouteHandler) {
	if r.node.getState() != gcluster.Shut {
		glog.Warnf("the node server is working, can't set default route handler")
		return
	}
...
func (r *Router) handle(req *request) {
	...
	if !ok && r.defaultRouteHandler == nil {
		...
		glog.Warnf("message routing does not register handler function, route: %v", req.message.Route)
		return
	}
