...
func (k *Key) marshalPublicKey(format Format, out io.Writer) (err error) {
	...
	default:
		return gerrors.New("invalid key format")
	}
...
func (k *Key) savePublicKey(format Format, dir string, file string) (err error) {
	base, _, name, ext := gpath.Split(file)
	if ext != "" {
		file = name + ".pub." + ext
	} else {
		file = name + ".pub"
...
func loadKey(key string) (*pem.Block, error) {
	...
	if gpath.IsFile(key) {
		...
		buffer = gconv.StringToBytes(key)
	}
...
func parsePublicKey(publicKey string) (*rsa.PublicKey, error) {
	...
	if black == nil {
		return nil, gerrors.New("invalid public key")
	}
...
func parsePrivateKey(privateKey string) (*rsa.PrivateKey, error) {
	...
	if black == nil {
		return nil, gerrors.New("invalid private key")
	}
