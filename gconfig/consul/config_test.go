...
func init() {
	source := consul.NewSource(consul.WithMode(gconfig.ReadWrite))
...
func TestLoad(t *testing.T) {
	...
	file := "config.json"
	c, err := gconfig.Load(ctx, consul.Name, file)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStore(t *testing.T) {
	...
	err := gconfig.Store(ctx, consul.Name, file, content1, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = gconfig.Store(ctx, consul.Name, file, content2)
	if err != nil {
		t.Fatal(err)
	}
}
