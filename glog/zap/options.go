...
func defaultOptions() *options {
	...
	file := getc.Get(zapFileKey, getc.Get(defaultFileKey).String()).String()
	if file != "" {
		opts.file = file
	}

	level := getc.Get(zapLevelKey, getc.Get(defaultLevelKey).String()).String()
	if lvl := glog.ParseLevel(level); lvl != glog.NoneLevel {
		opts.level = lvl
	}

	format := getc.Get(zapFormatKey, getc.Get(defaultFormatKey).String()).String()
	...
	timeFormat := getc.Get(zapTimeFormatKey, getc.Get(defaultTimeFormatKey).String()).String()
	if timeFormat != "" {
		opts.timeFormat = timeFormat
	}

	stackLevel := getc.Get(zapStackLevelKey, getc.Get(defaultStackLevelKey).String()).String()
	if lvl := glog.ParseLevel(stackLevel); lvl != glog.NoneLevel {
		opts.stackLevel = lvl
	}

	fileMaxAge := getc.Get(zapFileMaxAgeKey, getc.Get(defaultFileMaxAgeKey).Duration()).Duration()
	if fileMaxAge > 0 {
		opts.fileMaxAge = fileMaxAge
	}

	fileMaxSize := getc.Get(zapFileMaxSizeKey, getc.Get(defaultFileMaxSizeKey).Int64()).Int64()
	if fileMaxSize > 0 {
		opts.fileMaxSize = fileMaxSize
	}

	fileCutRule := getc.Get(zapFileCutRuleKey, getc.Get(defaultFileCutRuleKey).String()).String()
	...
	opts.stdout = getc.Get(zapStdoutKey, getc.Get(defaultStdoutKey, defaultStdout).Bool()).Bool()
	opts.callerFullPath = getc.Get(zapCallerFullPathKey, getc.Get(defaultCallerFullPathKey, defaultCallerFullPath).Bool()).Bool()
	opts.classifiedStorage = getc.Get(zapClassifiedStorageKey, getc.Get(defaultClassifiedStorageKey, defaultClassifiedStorage).Bool()).Bool()
