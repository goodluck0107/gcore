...
const (
	defaultFile              = "./log/gcore.log"
	defaultLevel             = glog.InfoLevel
	defaultFormat            = glog.TextFormat
	defaultStdout            = true
	defaultFileMaxAge        = 7 * 24 * time.Hour
	defaultFileMaxSize       = 100
	defaultFileCutRule       = glog.CutByDay
...
type options struct {
	...
	format            glog.Format   // 输出的日志格式，Text或者Json，默认Text
	...
	fileCutRule       glog.CutRule  // 文件切割规则，默认按照天
...
func defaultOptions() *options {
	...
	file := getc.Get(logrusFileKey, getc.Get(defaultFileKey).String()).String()
	if file != "" {
		opts.file = file
	}

	level := getc.Get(logrusLevelKey, getc.Get(defaultLevelKey).String()).String()
	if lvl := glog.ParseLevel(level); lvl != glog.NoneLevel {
		opts.level = lvl
	}

	format := getc.Get(logrusFormatKey, getc.Get(defaultFormatKey).String()).String()
	switch strings.ToLower(format) {
	case glog.JsonFormat.String():
		opts.format = glog.JsonFormat
	case glog.TextFormat.String():
		opts.format = glog.TextFormat
	}

	timeFormat := getc.Get(logrusTimeFormatKey, getc.Get(defaultTimeFormatKey).String()).String()
	if timeFormat != "" {
		opts.timeFormat = timeFormat
	}

	stackLevel := getc.Get(logrusStackLevelKey, getc.Get(defaultStackLevelKey).String()).String()
	if lvl := glog.ParseLevel(stackLevel); lvl != glog.NoneLevel {
		opts.stackLevel = lvl
	}

	fileMaxAge := getc.Get(logrusFileMaxAgeKey, getc.Get(defaultFileMaxAgeKey).Duration()).Duration()
	if fileMaxAge > 0 {
		opts.fileMaxAge = fileMaxAge
	}

	fileMaxSize := getc.Get(logrusFileMaxSizeKey, getc.Get(defaultFileMaxSizeKey).Int64()).Int64()
	if fileMaxSize > 0 {
		opts.fileMaxSize = fileMaxSize
	}

	fileCutRule := getc.Get(logrusFileCutRuleKey, getc.Get(defaultFileCutRuleKey).String()).String()
	switch strings.ToLower(fileCutRule) {
	case glog.CutByYear.String():
		opts.fileCutRule = glog.CutByYear
	case glog.CutByMonth.String():
		opts.fileCutRule = glog.CutByMonth
	case glog.CutByDay.String():
		opts.fileCutRule = glog.CutByDay
	case glog.CutByHour.String():
		opts.fileCutRule = glog.CutByHour
	case glog.CutByMinute.String():
		opts.fileCutRule = glog.CutByMinute
	case glog.CutBySecond.String():
		opts.fileCutRule = glog.CutBySecond
	}

	opts.stdout = getc.Get(logrusStdoutKey, getc.Get(defaultStdoutKey, defaultStdout).Bool()).Bool()
	opts.callerFullPath = getc.Get(logrusCallerFullPathKey, getc.Get(defaultCallerFullPathKey, defaultCallerFullPath).Bool()).Bool()
	opts.classifiedStorage = getc.Get(logrusClassifiedStorageKey, getc.Get(defaultClassifiedStorageKey, defaultClassifiedStorage).Bool()).Bool()
...
// 例如：InfoLevel级的日志将存储于gcore.debug.20220910.log、gcore.info.20220910.log两个日志文件中
