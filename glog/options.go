...
func defaultOptions() *options {
	...
	file := getc.Get(defaultFileKey).String()
	if file != "" {
		opts.file = file
	}

	level := getc.Get(defaultLevelKey).String()
	if lvl := ParseLevel(level); lvl != NoneLevel {
		opts.level = lvl
	}

	format := getc.Get(defaultFormatKey).String()
	...
	timeFormat := getc.Get(defaultTimeFormatKey).String()
	if timeFormat != "" {
		opts.timeFormat = timeFormat
	}

	stackLevel := getc.Get(defaultStackLevelKey).String()
	if lvl := ParseLevel(stackLevel); lvl != NoneLevel {
		opts.stackLevel = lvl
	}

	fileMaxAge := getc.Get(defaultFileMaxAgeKey).Duration()
	if fileMaxAge > 0 {
		opts.fileMaxAge = fileMaxAge
	}

	fileMaxSize := getc.Get(defaultFileMaxSizeKey).Int64()
	if fileMaxSize > 0 {
		opts.fileMaxSize = fileMaxSize
	}

	fileCutRule := getc.Get(defaultFileCutRuleKey).String()
	...
	opts.stdout = getc.Get(defaultStdoutKey, defaultStdout).Bool()
	opts.callerFullPath = getc.Get(defaultCallerFullPathKey, defaultCallerFullPath).Bool()
	opts.classifiedStorage = getc.Get(defaultClassifiedStorageKey, defaultClassifiedStorage).Bool()
