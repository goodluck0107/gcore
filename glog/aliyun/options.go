...
const (
	...
	defaultStackLevel     = glog.ErrorLevel
	defaultStdout         = true
	defaultSyncout        = true
...
func defaultOptions() *options {
	...
	level := getc.Match(aliyunLevelKey, defaultLevelKey).Get().String()
	if lvl := glog.ParseLevel(level); lvl != glog.NoneLevel {
		opts.level = lvl
	}

	stackLevel := getc.Match(aliyunStackLevelKey, defaultStackLevelKey).Get().String()
	if lvl := glog.ParseLevel(stackLevel); lvl != glog.NoneLevel {
		opts.stackLevel = lvl
	}

	opts.stdout = getc.Match(aliyunStdoutKey, defaultStdoutKey).Get(defaultStdout).Bool()
	opts.timeFormat = getc.Match(aliyunTimeFormatKey, defaultTimeFormatKey).Get(defaultTimeFormat).String()
	opts.callerFullPath = getc.Match(aliyunCallerFullPathKey, defaultCallerFullPathKey).Get(defaultCallerFullPath).Bool()
	opts.endpoint = getc.Get(aliyunEndpointKey).String()
	opts.accessKeyID = getc.Get(aliyunAccessKeyIDKey).String()
	opts.accessKeySecret = getc.Get(aliyunAccessKeySecretKey).String()
	opts.project = getc.Get(aliyunProjectKey).String()
	opts.logstore = getc.Get(aliyunLogstoreKey).String()
	opts.topic = getc.Get(aliyunTopicKey).String()
	opts.source = getc.Get(aliyunSourceKey).String()
	opts.syncout = getc.Get(aliyunSyncoutKey, defaultSyncout).Bool()
