...
func (r *registrar) register(ctx context.Context, ins *gregistry.ServiceInstance) error {
	...
	registration.Meta[metaFieldEvents] = gconv.Json(ins.Events)
	registration.Meta[metaFieldWeight] = gconv.String(ins.Weight)
	registration.Meta[metaFieldServices] = gconv.Json(ins.Services)
...
func (r *registrar) heartbeat(ctx context.Context, insID string) {
	...
	if err != nil {
		glog.Warnf("update heartbeat ttl failed: %v", err)
	}
