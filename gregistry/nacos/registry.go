	...
	if o.client == nil {
		...
		if len(param.ServerConfigs) == 0 {
			if err != nil {
				...
				r.err = gerrors.New("invalid server urls")
			}
		} else {
			if err != nil {
...
func parseInstances(instances []model.Instance) ([]*gregistry.ServiceInstance, error) {
	services := make([]*gregistry.ServiceInstance, 0, len(instances))
	for _, instance := range instances {
		...
		ins.Routes = make([]gregistry.Route, 0)
		ins.Events = make([]int, 0)
		ins.Services = make([]string, 0)
		ins.Weight = gconv.Int(instance.Metadata[metaFieldWeight])
