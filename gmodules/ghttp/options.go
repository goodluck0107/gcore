...
func defaultOptions() *options {
	opts := &options{
		name:     defaultName,
		addr:     defaultAddr,
		keyFile:  getc.Get(defaultKeyFileKey).String(),
		certFile: getc.Get(defaultCertFileKey).String(),
	...
	if err := getc.Get(defaultCorsKey).Scan(&opts.corsOpts); err != nil {
		opts.corsOpts = CorsOptions{}
	}

	if err := getc.Get(defaultSwaggerKey).Scan(&opts.swagOpts); err != nil {
