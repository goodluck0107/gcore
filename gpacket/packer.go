...
func NewPacker(opts ...Option) *defaultPacker {
	...
	if o.routeBytes != 1 && o.routeBytes != 2 && o.routeBytes != 4 {
		glog.Fatalf("the number of route bytes must be 1、2、4, and give %d", o.routeBytes)
	}

	if o.seqBytes != 0 && o.seqBytes != 1 && o.seqBytes != 2 && o.seqBytes != 4 {
		glog.Fatalf("the number of seq bytes must be 0、1、2、4, and give %d", o.seqBytes)
	}

	if o.bufferBytes < 0 {
		glog.Fatalf("the number of buffer bytes must be greater than or equal to 0, and give %d", o.bufferBytes)
	}
...
func (p *defaultPacker) ReadMessage(reader interface{}) ([]byte, error) {
	...
	default:
		return nil, gerrors.ErrInvalidReader
	}
}
...
func (p *defaultPacker) CheckHeartbeat(data []byte) (bool, error) {
	if len(data) < defaultSizeBytes+defaultHeaderBytes {
		return false, gerrors.ErrInvalidMessage
	}
