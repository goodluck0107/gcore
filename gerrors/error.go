...
// Code 返回错误码
func Code(err error) *gcodes.Code {
	if err != nil {
		if e, ok := err.(interface{ Code() *gcodes.Code }); ok {
			return e.Code()
		}
	}
...
// Replace 替换文本
func Replace(err error, text string, condition ...gcodes.Code) error {
	if err == nil {
		return nil
	}

	if e, ok := err.(interface {
		Replace(text string, condition ...gcodes.Code) error
	}); ok {
...
func (e *Error) Error() (text string) {
	...
	if e.code != nil && e.code != gcodes.OK {
		text = e.code.String()
	}
...
// Replace 替换文本
func (e *Error) Replace(text string, condition ...*gcodes.Code) error {
	if e == nil {
		return nil
	}
