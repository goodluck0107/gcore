...
// Subscribe 订阅事件
func (eb *Eventbus) Subscribe(ctx context.Context, topic string, handler geventbus.EventHandler) error {
	...
	if !ok {
		c = &consumer{handlers: make(map[uintptr][]geventbus.EventHandler, 1)}
...
// Unsubscribe 取消订阅
func (eb *Eventbus) Unsubscribe(ctx context.Context, topic string, handler geventbus.EventHandler) error {
	eb.rw.Lock()
	defer eb.rw.Unlock()
...
func (eb *Eventbus) watch() {
	for {
		...
		case *redis.Message:
			...
			eb.rw.RUnlock()
			if ok {
				c.dispatch(gconv.Bytes(v.Payload))
			}
		}
	}
}
