...
func defaultOptions() *options {
	...
	if size := getc.Get(defaultSizeKey).Int(); size > 0 {
		opts.size = size
	}

	opts.nonblocking = getc.Get(defaultNonblockingKey, defaultNonblocking).Bool()
	opts.disablePurge = getc.Get(defaultDisablePurgeKey, defaultDisablePurge).Bool()
