...
func (l *GateLinker) Locate(ctx context.Context, uid int64) (string, error) {
	if l.opts.Locator == nil {
		return "", gerrors.ErrNotFoundLocator
	}
	...
	if gid == "" {
		return "", gerrors.ErrNotFoundUserLocation
	}
...
func (l *GateLinker) FetchGateList(ctx context.Context, states ...gcluster.State) ([]*gregistry.ServiceInstance, error) {
	services, err := l.opts.Registry.Services(ctx, gcluster.Gate.String())
	if err != nil {
		return nil, err
	}
	...
	list := make([]*gregistry.ServiceInstance, 0, len(services))
...
// GetState 获取网关状态
func (l *GateLinker) GetState(ctx context.Context, gid string) (gcluster.State, error) {
	client, err := l.doBuildClient(gid)
	if err != nil {
		return gcluster.Shut, err
	}
...
// SetState 设置网关状态
func (l *GateLinker) SetState(ctx context.Context, gid string, state gcluster.State) error {
	client, err := l.doBuildClient(gid)
	if err != nil {
		return err
	}
...
func (l *GateLinker) GetIP(ctx context.Context, args *GetIPArgs) (string, error) {
	...
	default:
		return "", gerrors.ErrInvalidSessionKind
	}
}

// 直接获取IP
func (l *GateLinker) doDirectGetIP(ctx context.Context, gid string, kind gsession.Kind, target int64) (string, error) {
	client, err := l.doBuildClient(gid)
	if err != nil {
		return "", err
	}
...
func (l *GateLinker) doIndirectGetIP(ctx context.Context, uid int64) (string, error) {
	v, err := l.doRPC(ctx, uid, func(client *gate.Client) (bool, interface{}, error) {
		ip, miss, err := client.GetIP(ctx, gsession.User, uid)
		return miss, ip, err
	})
	if err != nil {
		return "", err
...
// Stat 统计会话总数
func (l *GateLinker) Stat(ctx context.Context, kind gsession.Kind) (int64, error) {
	total := int64(0)
...
func (l *GateLinker) IsOnline(ctx context.Context, args *IsOnlineArgs) (bool, error) {
	...
	default:
		return false, gerrors.ErrInvalidSessionKind
	}
}
...
func (l *GateLinker) doIndirectDisconnect(ctx context.Context, uid int64, force bool) error {
	_, err := l.doRPC(ctx, uid, func(client *gate.Client) (bool, interface{}, error) {
		return false, nil, client.Disconnect(ctx, gsession.User, uid, force)
	})
...
func (l *GateLinker) doDirectMulticast(ctx context.Context, args *MulticastArgs) error {
	if len(args.Targets) == 0 {
		return gerrors.ErrReceiveTargetEmpty
	}
...
func (l *GateLinker) doIndirectMulticast(ctx context.Context, args *MulticastArgs) error {
	...
	for _, target := range args.Targets {
		func(target int64) {
			eg.Go(func() error {
				message, err := gpacket.PackBuffer(&gpacket.Message{
...
func (l *GateLinker) doBuildClient(gid string) (*gate.Client, error) {
	if gid == "" {
		return nil, gerrors.ErrInvalidGID
	}
...
func (l *GateLinker) PackMessage(message *Message, encrypt bool) (buffer.Buffer, error) {
	...
	return gpacket.PackBuffer(&gpacket.Message{
		Seq:    message.Seq,
		Route:  message.Route,
		Buffer: buf,
	})
...
func (l *GateLinker) WatchUserLocate() {
	...
	watcher, err := l.opts.Locator.Watch(ctx, gcluster.Gate.String())
	cancel()
	if err != nil {
		glog.Fatalf("user locate event watch failed: %v", err)
	}
	...
	go func() {
		...
		for {
			...
			for _, event := range events {
				switch event.Type {
				case glocate.BindGate:
					l.sources.Store(event.UID, event.InsID)
				case glocate.UnbindGate:
...
func (l *GateLinker) WatchClusterInstance() {
	ctx, cancel := context.WithTimeout(l.ctx, 3*time.Second)
	watcher, err := l.opts.Registry.Watch(ctx, gcluster.Gate.String())
	cancel()
	if err != nil {
