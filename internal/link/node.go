...
func (l *NodeLinker) Ask(ctx context.Context, uid int64, name, nid string) (string, bool, error) {
	if l.opts.Locator == nil {
		return "", false, gerrors.ErrNotFoundLocator
	}
	...
	if insID == "" {
		return "", false, gerrors.ErrNotFoundUserLocation
	}
...
func (l *NodeLinker) Deliver(ctx context.Context, args *DeliverArgs) error {
	...
	if args.NID != "" {
		...
		if err != nil && !gerrors.Is(err, gerrors.ErrNotFoundUserLocation) {
			return err
		}
...
func (l *NodeLinker) FetchNodeList(ctx context.Context, states ...gcluster.State) ([]*gregistry.ServiceInstance, error) {
	services, err := l.opts.Registry.Services(ctx, gcluster.Node.String())
	if err != nil {
		return nil, err
	}
...
// GetState 获取节点状态
func (l *NodeLinker) GetState(ctx context.Context, nid string) (gcluster.State, error) {
...
// SetState 设置节点状态
func (l *NodeLinker) SetState(ctx context.Context, nid string, state gcluster.State) error {
	client, err := l.doBuildClient(nid)
	if err != nil {
		return err
	}
...
func (l *NodeLinker) doRPC(ctx context.Context, routeID int32, uid int64, fn func(ctx context.Context, client *node.Client) (bool, interface{}, error)) (interface{}, error) {
	...
	if l.opts.InsKind == gcluster.Gate && route.Internal() {
		return nil, gerrors.ErrIllegalRequest
	}
...
func (l *NodeLinker) doBuildClient(nid string) (*node.Client, error) {
	if nid == "" {
		return nil, gerrors.ErrInvalidNID
	}
...
func (l *NodeLinker) doPackMessage(message *Message, encrypt bool) ([]byte, error) {
	...
	return gpacket.PackMessage(&gpacket.Message{
		Seq:    message.Seq,
		Route:  message.Route,
		Buffer: buffer,
	})
...
func (l *NodeLinker) WatchUserLocate() {
	...
	watcher, err := l.opts.Locator.Watch(ctx, gcluster.Node.String())
	cancel()
	if err != nil {
...
func (l *NodeLinker) WatchClusterInstance() {
	ctx, cancel := context.WithTimeout(l.ctx, 3*time.Second)
	watcher, err := l.opts.Registry.Watch(ctx, gcluster.Node.String())
	cancel()
	if err != nil {
		glog.Fatalf("the cluster instance watch failed: %v", err)
	}
