...
func TestDispatcher_ReplaceServices(t *testing.T) {
	var (
		instance1 = &gregistry.ServiceInstance{
			ID:       "xc",
			Name:     "gate-3",
		...
		instance2 = &gregistry.ServiceInstance{
			ID:       "xa",
			Name:     "gate-1",
			Kind:     gcluster.Node.String(),
			Alias:    "gate-1",
		...
		instance3 = &gregistry.ServiceInstance{
			ID:       "xb",
			Name:     "gate-2",
			Kind:     gcluster.Node.String(),
			Alias:    "gate-2",
			State:    gcluster.Hang.String(),
			Endpoint: endpoint.NewEndpoint("grpc", "127.0.0.1:8002", false).String(),
			Events:   []int{int(gcluster.Disconnect)},
			Routes: []gregistry.Route{{
				ID:       1,
				Stateful: false,
			}, {
...
func BenchmarkDispatcher_WeightRoundRobin(b *testing.B) {
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			// 准备足够数量的实例
			testInstances := make([]*gregistry.ServiceInstance, bm.instanceCount)
			for i := 0; i < bm.instanceCount; i++ {
				if i < len(instances) {
					...
					testInstances[i] = &gregistry.ServiceInstance{
