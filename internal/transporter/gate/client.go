...
// GetIP 获取客户端IP
func (c *Client) GetIP(ctx context.Context, kind gsession.Kind, target int64) (string, bool, error) {
	seq := c.doGenSequence()
...
// Stat 推送广播消息
func (c *Client) Stat(ctx context.Context, kind gsession.Kind) (int64, error) {
	seq := c.doGenSequence()
...
// IsOnline 检测是否在线
func (c *Client) IsOnline(ctx context.Context, kind gsession.Kind, target int64) (bool, bool, error) {
	seq := c.doGenSequence()
...
// Disconnect 断开连接
func (c *Client) Disconnect(ctx context.Context, kind gsession.Kind, target int64, force bool) error {
	if force {
...
// Push 异步推送消息
func (c *Client) Push(ctx context.Context, kind gsession.Kind, target int64, message buffer.Buffer) error {
	return c.cli.Send(ctx, protocol.EncodePushReq(0, kind, target, message), target)
}

// Multicast 推送组播消息
func (c *Client) Multicast(ctx context.Context, kind gsession.Kind, targets []int64, message buffer.Buffer) error {
	return c.cli.Send(ctx, protocol.EncodeMulticastReq(0, kind, targets, message))
}

// Broadcast 推送广播消息
func (c *Client) Broadcast(ctx context.Context, kind gsession.Kind, message buffer.Buffer) error {
