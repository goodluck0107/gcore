...
func EncodeGetIPReq(seq uint64, kind gsession.Kind, target int64) buffer.Buffer {
...
// 协议：size + header + route + seq + session kind + target
func DecodeGetIPReq(data []byte) (seq uint64, kind gsession.Kind, target int64, err error) {
...
func EncodeGetIPRes(seq uint64, code uint16, ip ...string) buffer.Buffer {
	...
	if code == codes.OK && len(ip) > 0 && ip[0] != "" {
		writer.WriteUint32s(binary.BigEndian, gnet.IP2Long(ip[0]))
	}
