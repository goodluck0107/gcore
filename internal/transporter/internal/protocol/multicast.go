...
func EncodeMulticastReq(seq uint64, kind gsession.Kind, targets []int64, message buffer.Buffer) buffer.Buffer {
...
// 协议：size + header + route + seq + session kind + count + targets + <message packet>
func DecodeMulticastReq(data []byte) (seq uint64, kind gsession.Kind, targets []int64, message []byte, err error) {
