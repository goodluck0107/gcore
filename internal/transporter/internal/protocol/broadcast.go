...
func EncodeBroadcastReq(seq uint64, kind gsession.Kind, message buffer.Buffer) buffer.Buffer {
...
// 协议：size + header + route + seq + session kind + <message packet>
func DecodeBroadcastReq(data []byte) (seq uint64, kind gsession.Kind, message []byte, err error) {
