...
func EncodePushReq(seq uint64, kind gsession.Kind, target int64, message buffer.Buffer) buffer.Buffer {
...
// 协议：size + header + route + seq + session kind + target + <message packet>
func DecodePushReq(data []byte) (seq uint64, kind gsession.Kind, target int64, message []byte, err error) {
