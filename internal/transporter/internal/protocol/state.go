...
func EncodeGetStateRes(seq uint64, code uint16, state gcluster.State) buffer.Buffer {
	buf := buffer.NewNocopyBuffer()
...
// 协议：size + header + route + seq + code + cluster state
func DecodeGetStateRes(data []byte) (code uint16, state gcluster.State, err error) {
...
func EncodeSetStateReq(seq uint64, state gcluster.State) buffer.Buffer {
	buf := buffer.NewNocopyBuffer()
...
// 协议：size + header + route + seq + cluster state
func DecodeSetStateReq(data []byte) (seq uint64, state gcluster.State, err error) {
