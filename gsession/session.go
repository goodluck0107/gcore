...
type Session struct {
	rw    sync.RWMutex            // 读写锁
	conns map[int64]gnetwork.Conn // 连接会话（连接ID -> gnetwork.Conn）
	users map[int64]gnetwork.Conn // 用户会话（用户ID -> gnetwork.Conn）
}

func NewSession() *Session {
	return &Session{
		conns: make(map[int64]gnetwork.Conn),
		users: make(map[int64]gnetwork.Conn),
	}
}

// AddConn 添加连接
func (s *Session) AddConn(conn gnetwork.Conn) {
	s.rw.Lock()
	defer s.rw.Unlock()
...
// RemConn 移除连接
func (s *Session) RemConn(conn gnetwork.Conn) {
	s.rw.Lock()
	defer s.rw.Unlock()
...
func (s *Session) Has(kind Kind, target int64) (ok bool, err error) {
	...
	case User:
		_, ok = s.users[target]
	default:
		err = gerrors.ErrInvalidSessionKind
	}
...
func (s *Session) Multicast(kind Kind, targets []int64, msg []byte) (n int64, err error) {
	...
	var conns map[int64]gnetwork.Conn
	switch kind {
	case Conn:
		conns = s.conns
	case User:
...
func (s *Session) Stat(kind Kind) (int64, error) {
	...
	default:
		return 0, gerrors.ErrInvalidSessionKind
	}
}

// 获取会话
func (s *Session) conn(kind Kind, target int64) (gnetwork.Conn, error) {
	switch kind {
	case Conn:
		conn, ok := s.conns[target]
		if !ok {
			return nil, gerrors.ErrNotFoundSession
		}
		return conn, nil
	case User:
		...
		return conn, nil
	default:
		return nil, gerrors.ErrInvalidSessionKind
	}
}
